...
import traceback
# 设定用户名、密码
username = u"18611660049"
passwd = u"wangye13141314"
...
#_jc_save_toDate返程日期
# 实例中用的是：北京 到 长春 2018-01-31
from_station = u"%u5317%u4EAC%2CBJP"
to_station = u"%u957F%u6625%2CCCT"
# 时间格式2018-01-25
from_date = u"2018-01-31"
# 车次，选择第几趟，0则从上之下依次点击
order = 0
# 设定乘客姓名
ticketer = u"乘客姓名"
...
def login():
    ...
    sleep(1)
    bwr.find_by_text(u"账号登录").click()
    sleep(3)
    #fill填充搜索框的内容，username。name=loginUserDTO.user_name的元素。
    un = {"J-userName":username}
    # bwr.fill_form(form_id="J-login-code-loading", field_values=un)
    # bwr.fill("J-userName", username)
    sleep(1)
    # bwr.fill_form(form_id="J-password", field_values=passwd)
